...
from uscho_women._parameter_levels
where parameter='year'
);

drop table if exists uscho_women._factors;

create table uscho_women._factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
insert into uscho_women._factors
...
parameter,
avg(raw_factor)
from uscho_women._factors
group by parameter
);

update uscho_women._factors
set raw_factor=raw_factor-s.mean
from scale s
where s.parameter=uscho_women._factors.parameter;
