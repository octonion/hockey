...
#dbWriteTable(con,c("uscho_women","_parameter_levels"),parameter_levels,row.names=TRUE)
...
model <- gs ~ year+field+d_div+o_div+game_length+s(offense,bs="re")+s(defense,bs="re")+s(game_id,bs="re")

fit <- bam(model,data=g,family=poisson(link=log))
fit
summary(fit)
...
#dbWriteTable(con,c("uscho_women","_basic_factors"),as.data.frame(combined),row.names=TRUE)
