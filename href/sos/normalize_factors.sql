...
from href._parameter_levels
where parameter='year'
);

drop table if exists href._factors;

create table href._factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
from href._parameter_levels npl
left outer join href._basic_factors nbf
...
update href._factors
set raw_factor=raw_factor-s.mean
from scale s
where s.parameter=href._factors.parameter;
