...
from uscho._parameter_levels
where parameter='year'
);

drop table if exists uscho._factors;

create table uscho._factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
update uscho._factors
set raw_factor=raw_factor-s.mean
from scale s
where s.parameter=uscho._factors.parameter;
